...
let app = new App();app.addComponent('comp-navbar', `<div>
    <div class="toolbar">
        <div class="row">
            <div class="col-3">
            ...
            <div class="col-6">
                <div class="tb-container">
                    <div class="row">
                        <input class="toolbar-textbox fullwidth" in="searchq" 
                        click="clearph" keydown="searchboxKeyDown"/>
                    </div>
                </div>
            </div>
            <div class="col-3">
...
`, {searchq:"Search Products",clearph: function(_this, my){
        if(_this.searchq == "Search Products") {
            _this.searchq = "";
        }
    },searchboxKeyDown: function(_this, my){
        console.log(_this.searchq);
    },}, false);app.addComponent('comp-products', `<div>
    <div class="container centered" style="flex-flow: column;">
        <div class="row centered">
            <h4>Products</h4>
        </div>
        <div class="row centered" no-show="{noProducts}">
            <div class="card" id="product-list" style="max-height: 300px; overflow-y: auto;">
                <div out="products">
                    <div class="row card-section">
                        <div class="row">
                            <p>{name}</p>
                        </div>
                        <button class="btn-outline" click="handleEditProduct" 
                        style="top: 6px; position:relative;">Edit</button>
                    </div>
                </div>
            </div>
        </div><br/><br/>

        <div class="row centered" no-show="{subtabstate[0]}">
            <div class="card" style="max-height: 50px; padding-top: 10px;">
                <div class="row">
                    <div class="col-10">
                        <input class="txt-underlined" in="productName"/>
                    </div>
                    <div class="col-2" style="padding-left: 10px;">
                        <button class="btn-primary" click="handleAddProduct">Add</button>
                    </div>
                </div>
            </div>
        </div>
        <div class="row centered" no-show="{subtabstate[1]}">
            <div class="card" style="max-height: 100px; padding-top: 10px;">
                <div class="row card-section" style="padding-bottom: 10px;">
                    <div style="width: 100%;">
                        <input class="txt-underlined" in="modProductName"/>
                    </div>
                </div>
                <div class="row card-section" style= "padding-top: 10px; padding-bottom: 10px;">
                    <button class="btn-primary" click="handleModProductName">
                        Modify
                    </button>
                    <button class="btn-outline" style="margin-left: 10px;" click="handleRemoveProduct">
                        Remove
                    </button>
                    <button class="btn-outline" style="margin-left: 10px; float: right;"
                    click="handleCancelProductMod">Cancel</button>
                </div>
            </div>
        </div>
        <div class="row alert-error centered" no-show="{alerts.noErrors}">
            <p out="alerts.errMessage"></p>
        </div>
    </div>
</div>

`, {products:[],productName:"",nextProdID:0,modProductName:"",modProductID:-1,alerts:{
            noErrors: true,
            errMessage: ""
        },noProducts:true,subtabstate:[false, true],handleEditProduct: function(_this, my){
        _this.modProductName = my.name;
        _this.modProductID = my.id;
        _this.subtabstate = [true, false];
    },handleAddProduct: function(_this, my){
        ...
        if(_this.productName == "" || _this.productName == null) {
            _this.alerts = {
                noErrors : false,
        ...
        if(_this.products.find(prod => prod.name === _this.productName) != null) {
            _this.alerts = {
                noErrors : false,
        ...
        _this.productName = "";
    },handleModProductName: function(_this, my){
        var modproduct = _this.products.find((value, index, obj) => {
            if(value.id === _this.modProductID) return value;
        });

        if(modproduct!=null) {
            modproduct.name = _this.modProductName;
        }

        _this.subtabstate = [false, true];
        _this.noProducts = (_this.products.length == 0);
    },handleRemoveProduct: function(_this, my){
        _this.products = _this.products.filter((value, index, array) => {
            if(value.id !== _this.modProductID) return value;
        });
        _this.subtabstate = [false, true];
        _this.noProducts = (_this.products.length == 0);
    },handleCancelProductMod: function(_this, my){
        _this.subtabstate = [false, true];
    },}, false);app.addComponent('comp-tabstrip', `<div style="width: 100%;">
    <div no-show="{curTabStat[0]}">
        <comp-products></comp-products>
    </div>
    <div no-show="{curTabStat[1]}">
        Tab
    </div>
</div>`, {curTabStat:[],onRefresh: function(_this, my){
        _this.curTabStat = _this.props.tabstat;
    },}, false);app.addComponent('comp-app', `<div>
    <div class="sidenav-overlay faceIn" id="sidenav-overlay" no-show="{hideSideNav}" click="closeSideNav">
    </div>
    <div class="sidenav" no-show="{hideSideNav}">
        <div class="sidenav-item" click="changeTab">Item 1</div>
        <div class="sidenav-item">Item 2</div>
    </div>
    <comp-navbar searchq="{searchq}"></comp-navbar>
    <div class="container" style="top: 60px; position:relative;">
        <comp-tabstrip tabstat="{tabStat}"></comp-tabstrip>
    </div>
</div>
`, {tabStat:[false, true],tabTo:1,hideSideNav:true,searchq:"",changeTab: function(_this, my){
        _this.showSideNav(_this);
        for(var i=0;i<_this.tabStat.length;i++) {
            if(i == _this.tabTo) _this.tabStat[i] = false;
            else _this.tabStat[i] = true;
        }
    },showSideNav: function(_this, my){
        _this.hideSideNav = false;
    },closeSideNav: function(_this, my){
